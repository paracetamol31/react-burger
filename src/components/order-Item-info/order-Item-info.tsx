...
export const OrderItemInfo: FC = () => {
    const { ingredients, orderHistory, orderFeed } = useSelector((state: RootState) => state);
    const { id }: Readonly<Params<string>> = useParams();

    if (!id || !ingredients || !ingredients.ingredients) {
        return null;
    }
    ...
    let order: IOrderParams | null = orderFeed.orderData?.orders.find((item: IOrderParams) => item._id === id) || null;
    if (!order) {
        order = orderHistory.orderData?.orders.find((item: IOrderParams) => item._id === id) || null;
    }
        ...
        <section className={orderItemInfoStyles.wrapper}>
            ...
            <div className={`${orderItemInfoStyles.footer}`}>
                ...
                <div className={`${orderItemInfoStyles.totalCount} text text_type_digits-default`}>
                    {getTotalCaunt(order.ingredients, ingredients.ingredients)}
                    <CurrencyIcon type="primary" />
                </div>
            </div>
        </section>
